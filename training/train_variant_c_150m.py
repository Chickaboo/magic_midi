...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train Variant C at ~150M params on pre-tokenized Godzilla NPZ data. "
            "Designed for quality-first large-scale baseline runs."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="outputs/variant_c_150m",
    ...
    # Depth-fair ~150M production profile (Variant C).
...
def main() -> None:
    ...
    attn_heads = _resolve_divisible_heads(int(args.d_model), int(args.num_attention_heads))
    ...
    train_cfg = _build_train_config(
        args=args,
        checkpoint_dir=output_dir / "checkpoints" / "variant_c_150m",
    ...
    LOGGER.info(
        "Variant C production setup: params~150M target | d_model=%d n_layers=%d heads=%d ffn_exp=%d",
        int(args.d_model),
        int(args.n_layers),
        int(attn_heads),
        int(max(1, args.ffn_expansion)),
    )
    LOGGER.info(
    ...
    generation_out = (
        output_dir / "generated" / "variant_c_150m.mid" if seed_midi is not None else None
    )
    result = _run_variant(
    ...
    result_path = output_dir / "variant_c_150m_result.json"
    result_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    print("Variant C 150M run complete")
